...
/**
 * A block of real presence, built from heartbeats rather than app opens.
 *
 * Has its own table on purpose. Presence is attendance, not a product or money
 * change, so it must never land in `audit_logs` — that table is what a boss
 * opens to review unusual employee activity, and filling it with heartbeats
 * would bury the entries that matter.
 *
 * An in-progress block stays local (`synced: 1`, so the push skips it). Only
 * when the block CLOSES is it marked `synced: 0` and pushed — one row per real
 * stretch of presence, not one per app open.
 */
export interface WorkSession {
  ...
  user_id: string;
  started_at: string;
  /** Bumped by the heartbeat while the app is in the foreground. */
  last_seen_at: string;
  /** Foreground time only — excludes the app sitting backgrounded. */
  active_seconds: number;
  /** Set when the block is closed; absent while it is still open. */
  ended_at?: string;
  isDeleted: number;
  synced: number;
  updated_at: string;
...
export class PosDatabase extends Dexie {
  ...
  constructor() {
    ...
    this.version(21).stores({
      workSessions: 'id, shop_id, user_id, started_at, flushed, [shop_id+user_id]'
    });

    // v22: presence gets its own SYNCED table so attendance never has to be
    // written into audit_logs, which is the boss's suspicious-activity record.
    this.version(22).stores({
      workSessions: 'id, shop_id, user_id, started_at, synced, isDeleted, [shop_id+user_id], [shop_id+isDeleted+started_at]'
    });
