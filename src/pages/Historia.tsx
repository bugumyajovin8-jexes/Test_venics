...
import { isReadOnlyError } from '../services/readOnly';
...
import { Receipt, Calendar, TrendingUp, BarChart3, ArrowUpRight, ArrowDownRight, RotateCcw, AlertCircle, AlertTriangle, FileText, RefreshCw, Plus, Minus, Trash2, X, Search, ShoppingCart, CheckCircle, Tag, Wallet, ArrowLeft, Lock } from 'lucide-react';
...
export default function Historia() {
  ...
  // Header back-button guard. This page is where the iOS ghost click lands after tapping "back" on
  // a child page (e.g. Ripoti ya Siku) whose back button sits at the same top-left spot: the
  // synthesized click fires ~300ms after we mount and would trigger a SECOND navigate(-1), sending
  // the user one page too far (→ Dashboard). Ignoring a back trigger that fires within 500ms of
  // mount absorbs that ghost while leaving real taps (which come much later) untouched.
  const mountAtRef = useRef(Date.now());
  const handleHeaderBack = () => {
    if (Date.now() - mountAtRef.current < 500) return;
    navigate(-1);
  };
    ...
    try {
      ...
      await db.transaction('rw', [db.sales, db.saleItems, db.products, db.debtPayments, db.auditLogs, db.settings], async () => {
        ...
        await SyncService.logAction('refund_sale', {
          ...
          // The report reads "amerudisha mauzo yaliyofanyika tarehe …", which
          // needs the ORIGINAL sale date, not the moment of the refund.
      ...
      // The licence sheet is already on screen, raised by the database guard.
      // Adding a raw "read-only mode" alert on top of it shows the user the
      // internals twice and contradicts the calm message they were given.
  ...
  return (
    <div className={`p-4 flex flex-col h-full pt-safe pt-safe-standalone ${isMagnified ? 'overflow-y-auto' : ''}`}>
      <div className="flex items-center mb-4">
        <button
          onClick={tap(handleHeaderBack)}
          onPointerUp={tap(handleHeaderBack)}
        ...
        {hasMapatoAccess && (
          <button
            onClick={tap(() => { navigate('/ripoti-ya-siku'); })}
            onPointerUp={tap(() => { navigate('/ripoti-ya-siku'); })}
            className="ml-auto flex items-center gap-1.5 px-3 py-2 bg-blue-600 text-white rounded-xl shadow-sm text-xs font-bold cursor-pointer"
            style={{ WebkitTapHighlightColor: 'transparent' }}
          >
            <Calendar className="w-4 h-4" /> Ripoti ya Siku
          </button>
        )}
      </div>
      ...
      {view === 'risiti' ? (
        <>
          ...
          <div data-tour="sales-list" className="flex justify-between items-center mb-3">
                ...
                <div key={sale.id} className="bg-white p-4 rounded-xl shadow-sm border border-gray-100">
                  ...
                  <div className="flex justify-between items-end mt-3">
                    ...
                    <div className="flex flex-col items-end">
                      ...
                      {isAuthenticated && (
                        <button
                          onClick={tap(readOnly.guard(() => setReversingSaleId(sale.id)))}
                          onPointerUp={tap(readOnly.guard(() => setReversingSaleId(sale.id)))}
                          className={`flex items-center text-[10px] font-bold text-red-500 bg-red-50 px-2 py-1 rounded-lg transition-colors ${readOnly.lockedClass}`}
                        >
                          {readOnly.locked
        ...
        <div className="flex-1 overflow-y-auto space-y-6 pb-4 scrollbar-hide">
          ...
          {showReportModal && (
            <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50">
              ...
              <GhostClickGuard />
              <div className="bg-white rounded-2xl p-6 max-w-sm w-full shadow-xl">
                <div data-tour="period-filter" className="flex justify-between items-center mb-1">
      ...
      {showBackdatedSaleModal && (
        <div className="fixed inset-0 bg-gray-50 z-50 flex flex-col text-left overflow-hidden select-none pt-safe pt-safe-standalone">
          ...
          {/* Modal Content */}
          {backdatedIsCheckout ? (
            // Modal sits at z-50 (same as the bottom nav, which stays visible on top). Bottom
            // padding = nav height + safe area, so the confirm button clears the nav instead of
            // hiding behind it.
            <div className="p-4 pb-[calc(5rem+env(safe-area-inset-bottom))] flex flex-col flex-1 bg-white overflow-y-auto">
              ...
              {/* No flex-1 here: stretching this to fill pinned the confirm button to the bottom
                  with nothing to scroll, so the keyboard trapped it (same bug as Kikapu). Letting it
                  flow keeps the button just below the fields, above the keyboard. */}
              <div className="space-y-4">
          ...
          <div className="fixed bottom-[calc(4rem+env(safe-area-inset-bottom)+1rem)] right-4 z-40 flex flex-col items-end">
            {showActionMenu && (
              <div className="flex flex-col gap-3 mb-4 items-end animate-in fade-in slide-in-from-bottom-5">
                <button data-tour="backdated-expense-btn"
                  onClick={tap(() => {
                ...
                <button data-tour="backdated-sale-btn"
                  onClick={tap(() => {
                    setShowActionMenu(false);
                    setBackdatedSaleCart([]);
            ...
            {/* Guarded at the FAB rather than at the two actions inside it:
                both are writes, so opening a menu where every option is locked
                is a worse answer than explaining once. */}
            <button
              onClick={tap(readOnly.guard(() => setShowActionMenu(!showActionMenu)))}
              onPointerUp={tap(readOnly.guard(() => setShowActionMenu(!showActionMenu)))}
              className={`w-14 h-14 rounded-full flex items-center justify-center shadow-lg transition-all transform active:scale-95 z-50 ${showActionMenu ? 'bg-gray-800 rotate-45' : 'bg-blue-600'} text-white ${readOnly.lockedClass}`}
            >
              {readOnly.locked ? <Lock className="w-6 h-6" /> : <Plus className="w-7 h-7" />}
            </button>
          </div>
        </>
      )}
