...
import { Database, LogOut, RefreshCw, BarChart3, ChevronRight, Phone, Wallet, User, ShieldCheck, Trash2, Clock, AlertTriangle, X, CheckCircle, MessageSquare, Zap, Bell, Users, Plus, Shield, Settings, Ban, FileText, Store, Package, Lock } from 'lucide-react';
import { useReadOnly } from '../hooks/useReadOnly';
import { SUPPORT_PHONE } from '../services/readOnly';
...
import { endPresence } from '../services/presence';
...
export default function Zaidi() {
  ...
  const handleAddShop = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // A trial is granted once per owner, so a second shop legitimately gets
      // none. Tracked here so the success message tells the truth rather than
      // promising 14 days the server did not issue.
      ...
      if (originalShopId) {
        try {
          ...
          if (!tempErr) {
            updatedTemp = true;
            try {
              const { data, error } = await supabase.functions.invoke('init-license');
              trialGranted = !error && !!(data as { license?: unknown } | null)?.license;
            } catch (licenseErr) {
              console.warn('[Zaidi] init-license failed for the new shop:', licenseErr);
            }
          }
        } catch (err) {
      ...
      showAlert(
        'Imefanikiwa',
        trialGranted
          ? `Duka jipya "${newShopName}" limeundwa kikamilifu! Trial ya siku 14 imeanzishwa kwa ajili ya duka hili.`
          : `Duka jipya "${newShopName}" limeundwa kikamilifu. Ili kuanza kuuza kwenye duka hili unahitaji kifurushi — piga simu ${SUPPORT_PHONE}.`,
      );
      setNewShopName('');
  ...
  const handleLogout = async () => {
    ...
    try {
      // Close the presence block first so the shift ends at logout rather than
      // being left open and later swept with a stale end time.
      await endPresence();
  ...
  return (
    <div className="p-4 pb-20">
      ...
      <div className="space-y-6">
        ...
        <section data-tour="profile-section" className="bg-white p-5 rounded-2xl shadow-sm border border-gray-100">
        ...
        <section data-tour="expiry-section" className="bg-white p-4 rounded-2xl shadow-sm border border-gray-100">
          <div className="flex items-center justify-between">
            <div
              role="button"
        ...
        {isBoss() && (
          <section data-tour="staff-section" className="bg-white p-4 rounded-2xl shadow-sm border border-gray-100">
            <div className="flex items-center justify-between mb-4">
              ...
              <button data-tour="add-staff-btn"
                onClick={tap(readOnly.guard(() => { setShowInviteModal(true); }))}
                onPointerUp={tap(readOnly.guard(() => { setShowInviteModal(true); }))}
                className={`p-2 bg-green-600 text-white rounded-xl shadow-md transition-colors flex items-center space-x-1 ${readOnly.lockedClass}`}
              >
                {readOnly.locked ? <Lock className="w-4 h-4" /> : <Plus className="w-4 h-4" />}
            ...
            <div data-tour="permissions-section" className="flex items-center justify-between p-4 bg-blue-50 border border-blue-100 rounded-2xl mb-4">
        ...
        {isBoss() && (
          <section data-tour="shops-section" className="bg-white p-4 rounded-2xl shadow-sm border border-gray-100">
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center">
              ...
              {/* Creating a shop provisions a fresh trial for it, so a lapsed
                  licence must not reach this flow — otherwise renewing is
                  optional and this button is the way around it. */}
              <button
                onClick={tap(readOnly.guard(() => { setShowAddShopModal(true); }))}
                onPointerUp={tap(readOnly.guard(() => { setShowAddShopModal(true); }))}
                className={`p-2 bg-blue-600 text-white rounded-xl shadow-md transition-colors flex items-center space-x-1 cursor-pointer ${readOnly.lockedClass}`}
              >
                {readOnly.locked ? <Lock className="w-4 h-4" /> : <Plus className="w-4 h-4" />}
        ...
        {isBoss() && (
          <section data-tour="daily-reports-toggle" className="space-y-3">
        ...
        <section data-tour="receipt-settings" className="bg-white p-4 rounded-2xl shadow-sm border border-gray-100">
          <div className="flex items-center justify-between">
            <div
              role="button"
        ...
        {isBoss() && (
          <section data-tour="hours24-toggle" className="bg-white p-4 rounded-2xl shadow-sm border border-gray-100">
            <div className="flex items-center justify-between">
              <div
                role="button"
        ...
        {isBoss() && (
          <section data-tour="clear-history" className="bg-white p-4 rounded-2xl shadow-sm border border-gray-100">
        ...
        {isBoss() && (
          <section data-tour="license-section" className="bg-white p-5 rounded-2xl shadow-sm border border-gray-100 animate-in fade-in duration-300">
        ...
        <section data-tour="support-section" className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 text-center">
