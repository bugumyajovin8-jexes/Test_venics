...
export default function Kikapu() {
  ...
  const handleCompleteSale = async (paymentMethod: 'cash' | 'credit' | 'mobile') => {
    // Checked here rather than on the three payment buttons so every route into
    // a sale — cash, mobile, credit — is covered by one check.
    if (readOnly.locked) {
  ...
  if (isCheckout) {
    return (
      // Natural-flow, scrollable layout (NOT h-full + flex-1 pinning). Pinning the button to the
      // bottom left nothing to scroll, so when the on-screen keyboard overlaid the bottom of the
      // screen the "Kamilisha Mkopo" button was trapped behind it. Flowing the button right after
      // the fields keeps it above the keyboard, and the parent scroll area covers taller cases.
          ...
          <div className={cart.length <= 3 ? "sticky z-20 bg-gray-50 pb-2 space-y-1.5 -mx-4 px-4" : "space-y-1.5"} style={{ top: cart.length <= 3 ? '-16px' : 'auto', paddingTop: cart.length <= 3 ? '16px' : '0' }}>
            <div className="bg-white border border-gray-150 rounded-2xl p-2.5 shadow-xs flex flex-col shrink-0 animate-in fade-in duration-200">
              ...
              <div className="flex items-center justify-between px-1.5 py-0.5 text-[9px] font-extrabold text-gray-400 uppercase tracking-wider border-b border-gray-100 mb-1 shrink-0">
                ...
                <div data-tour="cart-list" className="text-right">Bei / Punguzo</div>
              </div>
            ...
            <div className="flex flex-wrap gap-1.5 shrink-0">
              {/* MKOPO opens the credit form rather than completing a sale, so it
                  needs its own guard — otherwise a lapsed shop fills in the
                  customer's details before being told it cannot save them. */}
              <button
                onClick={tap(readOnly.guard(() => {
                  if (activeKeypad) {
                    commitKeypad(activeKeypad);
                    setActiveKeypad(null);
                  }
                ...
                onPointerUp={tap(readOnly.guard(() => {
                  if (activeKeypad) {
                    commitKeypad(activeKeypad);
                    setActiveKeypad(null);
                  }
                ...
                className={`bg-orange-500 hover:bg-orange-600 text-white py-2.5 rounded-xl font-extrabold text-[10px] tracking-tight active:scale-95 shadow-xs flex flex-col items-center justify-center cursor-pointer select-none touch-manipulation flex-1 min-w-[85px] ${readOnly.lockedClass}`}
              ...
              <button
                ...
                className={`bg-blue-600 hover:bg-blue-700 text-white py-2.5 rounded-xl font-extrabold text-[10px] tracking-tight active:scale-95 shadow-xs flex flex-col items-center justify-center cursor-pointer select-none touch-manipulation flex-1 min-w-[85px] ${readOnly.lockedClass}`}
                title="Lipa kwa njia ya Mtandao wa Simu (M-Pesa, TigoPesa, AirtelMoney, n.k.)"
              >
                <span>UZA (SIMU/BANK)</span>
              </button>
              <button data-tour="checkout-btn"
                onClick={tap(() => handleCompleteSale('cash'))}
                onPointerUp={tap(() => handleCompleteSale('cash'))}
                className={`bg-emerald-600 hover:bg-emerald-700 text-white py-2.5 rounded-xl font-extrabold text-[10px] tracking-tight active:scale-95 shadow-xs flex flex-col items-center justify-center cursor-pointer select-none touch-manipulation flex-1 min-w-[85px] ${readOnly.lockedClass}`}
