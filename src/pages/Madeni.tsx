...
export default function Madeni() {
  ...
  return (
    <div className="p-4 flex flex-col h-full relative">
      ...
      <div data-tour="debts-list" className="bg-red-50 p-4 rounded-2xl border border-red-100 mb-6">
            ...
            return (
              <div key={debt.id} className="bg-white p-4 rounded-xl shadow-sm border border-gray-100">
                ...
                <div className="flex flex-wrap sm:flex-nowrap gap-2 pt-3 border-t border-gray-100">
                  <button
                    onClick={tap(readOnly.guard(() => { setSelectedDebt(debt); }))}
                    onPointerUp={tap(readOnly.guard(() => { setSelectedDebt(debt); }))}
                    className={`flex-1 min-w-[80px] flex items-center justify-center text-[11px] font-bold text-blue-600 bg-blue-50 py-2.5 rounded-xl transition-all ${readOnly.lockedClass}`}
                  >
                    <Plus className="w-3.5 h-3.5 mr-1" />
                    Lipa Kidogo
                  </button>
                  <button
                    onClick={tap(readOnly.guard(() => { handleFullPayment(debt); }))}
                    onPointerUp={tap(readOnly.guard(() => { handleFullPayment(debt); }))}
                    className={`flex-1 min-w-[80px] flex items-center justify-center text-[11px] font-bold text-green-600 bg-green-50 py-2.5 rounded-xl transition-all ${readOnly.lockedClass}`}
                  >
