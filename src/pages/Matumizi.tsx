...
import { Plus, Trash2, Calendar, Tag, FileText, Wallet, ChevronDown, ChevronUp, ArrowLeft, Lock } from 'lucide-react';
  ...
  return (
    <div className="p-4 flex flex-col h-full pt-safe pt-safe-standalone">
      <div className="flex justify-between items-center mb-6">
        ...
        <button data-tour="add-expense-btn"
          onClick={tap(readOnly.guard(() => setIsAdding(true)))}
          onPointerUp={tap(readOnly.guard(() => setIsAdding(true)))}
          className={`bg-blue-600 shadow-blue-100 text-white p-3 rounded-2xl shadow-lg cursor-pointer touch-manipulation select-none transition-all ${readOnly.lockedClass}`}
          style={{
            WebkitTapHighlightColor: 'transparent',
            WebkitTouchCallout: 'none',
            touchAction: 'manipulation'
                        ...
                        <div className="text-right flex flex-col items-end">
                          ...
                          {(user?.role === 'boss') && (
                            <button
                              ...
                              className="mt-2 flex items-center gap-1 text-red-600 bg-red-50 border border-red-100 px-2.5 py-1 rounded-lg text-[10px] font-bold active:scale-95 transition-all cursor-pointer touch-manipulation select-none"
                              style={{ WebkitTapHighlightColor: 'transparent' }}
                            >
                              <Trash2 className="w-3.5 h-3.5" /> Futa
                            </button>
                          )}
                        </div>
                      </div>
