...
import { getValidStock, getSales30DaysVelocityMap, isLowStock, isProductStockTracked, isBatchExpired, isBatchExpiringSoon } from '../utils/stock';
...
export default function Dashibodi() {
  ...
  const expiredBatchesCount = isExpiryEnabled ? products.reduce((count, p) => {
    return count + (p.batches?.filter(b => Number(b.stock) > 0 && isBatchExpired(b.expiry_date)).length || 0);
  }, 0) : 0;

  const expiringSoonBatchesCount = isExpiryEnabled ? products.reduce((count, p) => {
    return count + (p.batches?.filter(b => Number(b.stock) > 0 && isBatchExpiringSoon(b.expiry_date, shop?.notify_expiry_days ?? 30)).length || 0);
  }, 0) : 0;
  ...
  const lowStockProducts = products.filter(p => {
    if (!isProductStockTracked(p, shop)) return false; // untracked products can't be "low on stock"
    return isLowStock(p, isExpiryEnabled, velocityMap);
  });

  // License calculation
  // Measured against the effective (end-of-day) expiry, like every other licence
  // reading in the app — otherwise this counter says "Siku 0" while the shop is
  // still happily selling through its final evening.
  const daysRemaining = license
    ? Math.max(0, Math.ceil((effectiveExpiry(license.expiryDate) - Date.now()) / (24 * 60 * 60 * 1000)))
    : 0;

  // Chart data (last 7 days) — memoized so the chart isn't fed a new array
  // reference on every render (which would restart the Recharts animation).
  ...
  const handleLicenseSync = async () => {
    ...
    try {
      ...
      // bypassFloor: an explicit tap must never be swallowed by the anti-spam
      // floor — someone who has just paid is standing there watching.
      await LicenseService.syncLicense(true, { bypassFloor: true });
      LicenseService.clearStatusCache();

      // Re-evaluate BEFORE syncing data. checkStatus publishes the new verdict,
      // which is what actually unlocks the buttons — without it the day count
      // refreshed from Dexie while the write gate stayed locked until a
      // background tick happened to run. It also has to come first because a
      // still-locked shop skips the data sync entirely.
  ...
  const handleVerifyProductPricing = async (productId: string) => {
    if (readOnly.locked) { readOnly.showLockedSheet(); return; }
    try {
  ...
  return (
    <div className="p-4 space-y-6">
      ...
      {/* Staff land here, so the explanation has to be on this page too — not
          only on the boss's dashboard. */}
      <ReadOnlyNotice />
            ...
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={chartData}>
                ...
                <Tooltip 
                  ...
                  cursor={{fill: '#f3f4f6'}}
                />
                <Bar dataKey="Mapato" fill="#3b82f6" radius={[4, 4, 0, 0]} isAnimationActive={false} />
              </BarChart>
            </ResponsiveContainer>
          </div>
        </div>
            ...
            >
              ...
              <div>
                ...
                <p className="text-xs text-orange-600 mt-1">
                  Kuna batches {expiringSoonBatchesCount} zitakwisha muda ndani ya siku {shop?.notify_expiry_days ?? 30}.
                </p>
              </div>
            </div>
          )}
