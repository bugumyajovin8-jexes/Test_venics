/**
 * Reading receipts and shelves with Venics Smart.
 *
 * The Gemini API key is NOT here and must never be. It lives as the Supabase
 * secret GEMINI_API_KEY, read only inside the `gemini` Edge Function, which
 * also enforces the licence check, the kill switch, the per-shop daily quota,
 * the model allowlist and the payload size caps. A key in the client bundle
 * bypasses every one of those — and a bundle is public, so shipping it is
 * publishing it.
 */

import { Type } from "@google/genai";
import { generateContent as generateContentViaProxy, GeminiGuardError } from "./geminiProxy";
import { VISION_MODEL } from "./aiModels";
...
function translateGeminiError(error: any, defaultMsg: string): Error {
  ...
  // A guard refusal (quota reached, AI disabled, image too large) already
  // carries a message written for the shop owner. Rewriting it here would tell
  // them the photo was unreadable when in fact they hit today's limit.
  if (error instanceof GeminiGuardError || error?.reason) {
  ...
  // 4. Everything else gets the caller's own wording, which knows whether this
  //    was a receipt being read or a shelf being counted.
...
export async function extractProductsFromImage(base64Image: string, mimeType: string): Promise<ExtractedProduct[]> {
  try {
    ...
    if (!response.text) {
      throw new Error("Venics Smart haikuweza kusoma chochote kwenye picha hii.");
    }
