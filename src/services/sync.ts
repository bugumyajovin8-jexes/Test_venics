...
import { isReadOnly, runAsSystemWrite } from './readOnly';
...
const BACKGROUND_TABLES = ['audit_logs', 'expenses', 'work_sessions'] as const;
const ALL_TABLES = [
  'shops',
  'users',
  'products',
...
export class SyncService {
  ...
  private static readonly PULL_THROTTLE_MS: Record<string, number> = {
    ...
    work_sessions: 300_000, // Attendance blocks close a few times a day (5 min)
  };

  /**
   * Settings tables already warned about this session. A rejected settings row
   * stays `synced: 0` and is retried on every cycle, so without this the boss
   * gets the same toast every few minutes for one bad row.
   */
  private static warnedSettingsPush = new Set<string>();
  ...
  private static async runOneSync(force: boolean, scope: SyncScope): Promise<void> {
    ...
    try {
      ...
      // The licence sync runs on EVERY scope while read-only, because it is the
      // only channel a renewal can arrive through once everything else is paused.
      if (scope === 'full' || scope === 'background' || isReadOnly()) {
        await this.runWithRetry(() => LicenseService.syncLicense(), 'syncLicense');
      }

      // A lapsed shop syncs nothing else.
      //
      // Push is paused rather than attempted-and-rejected. That matters: the
      // permanent-RLS handler in pushTable() marks rejected rows `synced: 1` to
      // stop a sync storm, so letting a lapsed shop push into a server-side
      // licence check would silently discard the very sales queued for flushing
      // on renewal. Not asking means never being refused.
      //
      // Pull is paused because every device in the shop is read-only, so no new
      // data exists anywhere to fetch — it would be pure egress against zero
      // revenue.
      if (isReadOnly()) {
        await this.restoreIfEmpty(shopId, user.role);
        useStore.getState().setSyncHealth('healthy');
        console.log(`${scope} sync skipped: shop is read-only (licence lapsed).`);
        return;
      }
        ...
        if (table) {
          ...
          if (unsyncedCount > 0) {
            try {
              ...
              anyPushed = true;
              // Recovered — allow a future failure to speak up again.
              this.warnedSettingsPush.delete(tableName);
            } catch (error: any) {
              const isNonCritical = ['saas_telemetry', 'audit_logs', 'assistant_chats'].includes(tableName);

              // Settings tables are pushed FIRST (shops, users, features lead
              // ALL_TABLES), so letting one wedge the run meant a single bad
              // settings row silently stopped sales and products from syncing
              // too. A rejected setting is a problem for that setting alone.
              const isSettingsTable = ['shops', 'users', 'features'].includes(tableName);

              if (isNonCritical) {
                console.warn(`[SyncService] Bypassed non-critical push failure for ${tableName} to keep main sync healthy:`, error);
              } else if (isSettingsTable) {
                console.error(
                  `[SyncService] Push REJECTED for ${tableName} — this change will not reach the server. ` +
                  `Most often a column in the pushed row does not exist remotely, or a policy refused the write. ` +
                  `Server said:`, error?.code, error?.message, error,
                );
                // Without this the boss changes a setting, sees it revert on the
                // next pull, and nothing anywhere says why. Once per table per
                // session: the row keeps retrying, and repeating the same toast
                // every cycle would be nagging, not informing.
                if (!this.warnedSettingsPush.has(tableName)) {
                  this.warnedSettingsPush.add(tableName);
                  useStore.getState().showToast(
                    'Mabadiliko ya mipangilio hayakuhifadhiwa kwenye seva. Jaribu tena au piga simu kwa msaada.',
                    'error',
                  );
                }
              } else {
  ...
  /** Shops already restored this session, so the pull is attempted once per shop. */
  private static restoreAttemptedFor: string | null = null;

  /**
   * The one exception to the read-only sync pause.
   *
   * A boss who reinstalls the app, or signs in on a new phone, while the licence
   * is lapsed would otherwise open a completely empty database — no history, no
   * reports, nothing. That is a worse outcome than the blocking screen this
   * whole feature exists to remove, so an empty local database earns exactly one
   * full pull. It downloads only the shop's own data, and read-only still
   * applies, so it grants a pirate nothing.
   */
  private static async restoreIfEmpty(shopId: string, role?: string) {
    if (this.restoreAttemptedFor === shopId) return;
    this.restoreAttemptedFor = shopId;

    const [productCount, saleCount] = await Promise.all([
      db.products.where('shop_id').equals(shopId).count(),
      db.sales.where('shop_id').equals(shopId).count(),
    ]);
    if (productCount > 0 || saleCount > 0) return;

    console.log('[SyncService] Lapsed licence with an empty local database — running one-time restore pull.');

    const settings = await db.settings.get(1);
    await runAsSystemWrite(async () => {
      for (const tableName of this.getPullTargets('full', role)) {
        await this.pullTable(
          tableName,
          this.getTableRef(tableName),
          shopId,
          this.getTableSyncDate(settings, tableName),
          true,
        );
      }
    });
  ...
  private static mapToRemote(tableName: string, data: any) {
    ...
    const tablesWithIsDeleted = ['products', 'sales', 'sale_items', 'expenses', 'debt_payments', 'audit_logs', 'work_sessions'];
