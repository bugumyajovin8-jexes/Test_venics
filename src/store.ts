...
import { saveWebSession, clearWebSession } from './utils/webSession';
...
export const useStore = create<PosState>((set, get) => ({
  ...
  setAuth: (token, user, refreshToken) => {
    if (token && user) {
      setDurable('pos_token', token);
      if (refreshToken) setDurable('pos_refresh_token', refreshToken);
      setDurable('pos_user', JSON.stringify(user));
      // Mirror the refresh token into an HttpOnly cookie so a PWA (esp. iOS) can
      // restore the session after the browser evicts localStorage. No-op on native.
  ...
  updateUser: (userUpdates) => set((state) => {
    ...
    setDurable('pos_user', JSON.stringify(updatedUser));
