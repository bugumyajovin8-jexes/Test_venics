...
import { supabaseAuthStorage } from './utils/durableStorage';
...
export const supabase = createClient(supabaseUrl, supabaseAnonKey, {
  auth: {
    // Durable storage: mirrors the session into native Preferences so it survives
    // browser/WebView storage eviction (the cause of the "logged out after a week"
    // problem). The default storageKey is kept, so existing sessions still load.
