        ...
        if (res.product_id && res.discrepancy !== 0) {
          ...
          if (product) {
            await db.products.update(res.product_id, {
              ...
              // A physical audit is the clearest count of all — send it as one,
              // handing the pending difference over to the count. See
              // db.ts Product.count_id.
              stock_delta: 0,
              count_id: uuidv4(),
              counted_stock: res.actual_stock,
              counted_base: res.expected_stock,
              counted_delta: product.stock_delta || 0,
              updated_at: nowIso(),
              synced: 0
            });
