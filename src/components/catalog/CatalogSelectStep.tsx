...
const COLS = 2; // two products per virtualized row — uses the desktop width
...
function Row({ index, style, rows, selected, owned, toggle, currency }: RowComponentProps<RowProps>) {
  const rowItems = rows[index] || [];
  return (
    <div
      style={{ ...style, display: 'grid', gridTemplateColumns: `repeat(${COLS}, minmax(0, 1fr))`, gap: '8px', paddingBottom: '8px' }}
    >
      {rowItems.map((item) => {
...
export default function CatalogSelectStep({
  ...
  const searchFiltered = useMemo(() => {
    ...
    if (!q) return items;
    return items.filter((i) => i.name.toLowerCase().includes(q) || (i.sub_category || '').toLowerCase().includes(q));
  }, [items, query]);

  const ownedCount = useMemo(() => searchFiltered.reduce((n, i) => n + (owned.has(i.id) ? 1 : 0), 0), [searchFiltered, owned]);
  ...
  const rows = useMemo(() => {
    const r: CatalogItem[][] = [];
    for (let i = 0; i < shown.length; i += COLS) r.push(shown.slice(i, i + COLS));
    return r;
  }, [shown]);
      ...
      <div ref={wrapRef} className="flex-1 min-h-0">
        {rows.length > 0 ? (
          <List
            ...
            rowHeight={72}
            rowComponent={Row}
            rowProps={{ rows, selected, owned, toggle, currency }}
