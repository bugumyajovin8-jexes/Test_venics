    ...
    <div className="fixed inset-0 z-50 bg-gray-50 flex flex-col">
      <div className="flex-1 min-h-0 flex flex-col overflow-hidden">
        ...
        <div className="flex items-center gap-2 px-4 py-3 border-b border-gray-200 bg-white">
          {canBack ? (
            <button type="button" onClick={goBack} className="p-1 text-gray-600 hover:text-gray-900"><ArrowLeft className="w-5 h-5" /></button>
          ) : (
            <Package className="w-5 h-5 text-blue-600" />
          )}
          <h2 className="flex-1 font-bold text-gray-900">{titles[step]}</h2>
          <button type="button" onClick={onClose} className="p-1 text-gray-500 hover:text-gray-800"><X className="w-5 h-5" /></button>
        </div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-4 max-w-2xl mx-auto w-full">
              {CATALOG_CATEGORIES.map((c) => (
                <button
                  ...
                  type="button"
                  disabled={!c.active}
                  onClick={() => c.active && pickCategory(c.key)}
                  className={`flex flex-col items-center gap-2 p-5 rounded-2xl border text-center transition-colors ${
                    c.active ? 'bg-white border-gray-200 hover:border-blue-400 hover:bg-blue-50' : 'bg-gray-100 border-gray-100 opacity-60 cursor-not-allowed'
                  }`}
                >
        ...
        {step === 'select' && (
          <div className="px-6 py-3 border-t border-gray-200 bg-white flex items-center justify-between">
            <span className="text-sm text-gray-500">{selected.size} zimechaguliwa</span>
            <button
              type="button"
              disabled={selected.size === 0}
              onClick={goToDetails}
              className="px-10 py-2.5 rounded-xl bg-blue-600 text-white font-bold disabled:bg-gray-300 hover:bg-blue-700 transition-colors"
            >
        ...
        {step === 'details' && (
          <div className="px-6 py-3 border-t border-gray-200 bg-white flex items-center justify-between gap-4">
            <span className="text-sm text-gray-500">
              Bidhaa {drafts.length} · Thamani ya hisa/Mtaji: <span className="font-semibold text-gray-900">{currency} {totalValue.toLocaleString()}</span>
            </span>
            <button
              type="button"
              disabled={drafts.length === 0}
              onClick={save}
              className="px-10 py-2.5 rounded-xl bg-green-600 text-white font-bold disabled:bg-gray-300 hover:bg-green-700 transition-colors shrink-0"
            >
        ...
        {step === 'done' && (
          <div className="px-6 py-3 border-t border-gray-200 bg-white flex justify-end">
            <button type="button" onClick={onClose} className="px-10 py-2.5 rounded-xl bg-blue-600 text-white font-bold hover:bg-blue-700 transition-colors">Funga</button>
          </div>
        )}
      </div>
    </div>
