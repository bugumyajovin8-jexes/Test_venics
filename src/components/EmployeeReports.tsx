...
import { getEmployeeTotals } from '../services/summaries';
import { 
  Users, 
  User, 
  ArrowLeft, 
...
import { format, isWithinInterval, startOfDay, endOfDay, differenceInHours, differenceInMinutes, startOfWeek, endOfWeek, startOfMonth, endOfMonth, subMonths, subDays } from 'date-fns';
import { motion } from 'motion/react';
import { formatCurrency } from '../utils/format';
import GhostClickGuard from './GhostClickGuard';
import { buildPresenceDay, formatDuration, type PresenceDay } from '../utils/presenceModel';

/**
 * One WORKING DAY, not one app session.
 *
 * The old model opened a "shift" on the first app_opened and closed it only on
 * an explicit logout — which almost never happens — so a day of short visits
 * became a single unbroken shift running to "now". A day now carries its real
 * presence blocks and the gaps between them.
 */
interface Shift {
  id: string;
  loginTime: Date;
...
export default function EmployeeReports({ onClose }: { onClose: () => void }) {
  ...
  // Raw shift-detail queries (sales/audit/expenses) are capped to the last 90 days
  // so "Wakati Wote" never loads years of raw sales. The all-time employee TOTALS
  // come from the salesEmployeeDaily rollups (employeeTotalsMap) instead.
  const minDateIso = useMemo(() => {
    const now = new Date();
    const cap = subDays(now, 90);
    let start: Date;
    if (filterPeriod === 'this_week') start = startOfWeek(now, { weekStartsOn: 1 });
    else if (filterPeriod === 'this_month') start = startOfMonth(now);
    else if (filterPeriod === 'last_month') start = startOfMonth(subMonths(now, 1));
    else start = cap; // all_time → capped to the recent window for shift detail

    // The leaderboard ALWAYS reports the last 7 days, but it reads the same
    // `sales` query as the shift detail. If this window started later than 7
    // days ago (e.g. "Wiki Hii" starts on Monday), every sale between 7 days
    // ago and Monday was never loaded — so the leaderboard silently read 0,
    // then "filled in" once a wider filter happened to load that history.
    // The window must therefore never begin later than the leaderboard's own.
    const leaderboardFloor = startOfDay(subDays(now, 7));
    if (start.getTime() > leaderboardFloor.getTime()) start = leaderboardFloor;

    return (start.getTime() < cap.getTime() ? cap : start).toISOString();
  }, [filterPeriod]);

  // Selected period as YYYY-MM-DD, used to read all-time-capable per-employee
  // totals from the rollups.
  const periodDateRange = useMemo(() => {
    ...
    else if (filterPeriod === 'last_month') { const lm = subMonths(now, 1); start = startOfMonth(lm); end = endOfMonth(lm); }
    else start = new Date(0); // all_time
    return { start: format(start, 'yyyy-MM-dd'), end: format(end, 'yyyy-MM-dd') };
  }, [filterPeriod]);

  const employeeTotalsMap = useLiveQuery(() => {
    if (!user?.shopId) return new Map<string, { revenue: number; profit: number; count: number }>();
    return getEmployeeTotals(user.shopId, periodDateRange.start, periodDateRange.end);
  }, [user?.shopId, periodDateRange.start, periodDateRange.end]);
  ...
  const workSessions = useLiveQuery(() => {
    ...
    return db.workSessions
      .where('[shop_id+isDeleted+started_at]')
      .between([user.shopId, 0, minDateIso], [user.shopId, 0, '￿'])
      .toArray();
  }, [user?.shopId, minDateIso]) || [];
  ...
  const employeeShifts = useMemo(() => {
    ...
    const dayKey = (value: string | Date) => format(new Date(value), 'yyyy-MM-dd');

    const userLogs = auditLogs.filter(l => l.user_id === selectedEmployeeId);
    const userSales = sales.filter(s => s.user_id === selectedEmployeeId);
    const userExpenses = expenses.filter(e => (e as any).user_id === selectedEmployeeId);
    const userSessions = workSessions.filter(w => w.user_id === selectedEmployeeId);

    // Every day this employee left any trace at all.
    const days = new Set<string>();
    userLogs.forEach(l => days.add(dayKey(l.created_at)));
    userSales.forEach(s => days.add(dayKey(s.created_at)));
    userExpenses.forEach(e => days.add(dayKey(e.created_at || e.date)));
    userSessions.forEach(w => days.add(dayKey(w.started_at)));

    const shifts: Shift[] = [];

    for (const key of days) {
      const dayLogs = userLogs.filter(l => dayKey(l.created_at) === key);
      const daySales = userSales.filter(s => dayKey(s.created_at) === key);
      const dayExpenses = userExpenses.filter(e => dayKey(e.created_at || e.date) === key);
      // DebtPayment carries no user_id, so these stay day-scoped (shop-wide).
      const dayDebtPayments = debtPayments.filter(dp => dayKey(dp.created_at || dp.date) === key);

      // Real work counts as presence, so a recorded sale can never land inside a gap.
      const presence = buildPresenceDay(dayLogs, {
        sessions: userSessions.filter(w => dayKey(w.started_at) === key),
        activityAt: [
          ...daySales.map(s => s.created_at),
          ...dayExpenses.map(e => e.created_at || e.date),
          ...dayDebtPayments.map(dp => dp.created_at || dp.date),
        ],
      });

      const anchor = presence.firstSeen
        ?? (daySales[0] ? startOfDay(new Date(daySales[0].created_at)) : startOfDay(new Date(`${key}T12:00:00`)));

      shifts.push({
        id: key,
        loginTime: presence.firstSeen ?? anchor,
        logoutTime: presence.lastSeen,
        sales: daySales,
        expenses: dayExpenses,
        debtPayments: dayDebtPayments,
        audits: dayLogs.filter(l => !['login', 'logout', 'app_opened'].includes(l.action)),
        revenue: daySales.reduce((acc, s) => acc + (s.total_amount || 0), 0),
        profit: daySales.reduce((acc, s) => acc + (s.total_profit || 0), 0),
        presence,
      });
    }
    ...
    // Original behaviour, kept deliberately: list only days that actually had
    // sales, plus the most recent day (which may be a shift still in progress).
    // Note this also hides a day whose only activity was a refund or a price
    // edit — those remain visible in Mabadiliko ya Bidhaa.
    const chronological = [...shifts].sort((a, b) => a.loginTime.getTime() - b.loginTime.getTime());
    const withSales = chronological.filter(
      (s, i) => s.sales.length > 0 || i === chronological.length - 1,
    );

    return withSales
      .filter(s => s.loginTime >= startDate && s.loginTime <= endDate)
      .sort((a, b) => b.loginTime.getTime() - a.loginTime.getTime());
  }, [selectedEmployeeId, auditLogs, workSessions, sales, expenses, debtPayments, filterPeriod]);

  const summary = useMemo(() => {
    // Hours worked are the SUM OF PRESENCE BLOCKS, not first-seen to last-seen.
    // The old calculation counted absences as worked time, so an employee who
    // came at 08:00, left for four hours and returned at 14:00 was credited
    // with the whole span.
    const minutes = employeeShifts.reduce(
      (acc, shift) => acc + Math.round(shift.presence.activeSeconds / 60),
      0,
    );
    // Revenue/profit/count for the selected period come from the rollups, so they
    // stay accurate for "Wakati Wote" without loading all raw sales.
    const emp = selectedEmployeeId
      ? (employeeTotalsMap?.get(selectedEmployeeId) || { revenue: 0, profit: 0, count: 0 })
      : { revenue: 0, profit: 0, count: 0 };
    return { revenue: emp.revenue, profit: emp.profit, salesCount: emp.count, minutes };
  }, [employeeShifts, employeeTotalsMap, selectedEmployeeId]);

  // --- Loading feedback for period switches -----------------------------------
  // Revenue/profit/count come from the rollups and minutes from the 90-day shift
  // window; both re-query when the period (esp. "Wakati Wote") changes, and
  // useLiveQuery holds the old numbers meanwhile — which looks stuck. Show skeletons
  // from the switch until the new period's data settles.
  const [loadedPeriod, setLoadedPeriod] = useState('');
  useEffect(() => {
    setLoadedPeriod(filterPeriod);
  }, [employeeTotalsMap, sales, auditLogs]);
  const summaryLoading = !!selectedEmployeeId && (employeeTotalsMap === undefined || loadedPeriod !== filterPeriod);
  const empSkel = (w: string) => (
    <span className={`inline-block h-6 ${w} bg-gray-200 rounded animate-pulse align-middle`} />
  );
  ...
  const leaderboardData = useMemo(() => {
    ...
    const weeklySales = sales.filter(s => {
      ...
      // A refund sets BOTH isDeleted=1 and status='refunded', and `sales` is
      // queried on isDeleted=0 — so refunds are already gone before this runs.
      // The status checks are belt-and-braces for any row that was marked
      // without being soft-deleted.
      return saleTime >= sevenDaysAgo && s.status !== 'cancelled' && s.status !== 'refunded';
    });
          ...
          <div className="bg-white p-5 rounded-[2rem] shadow-sm border border-gray-100 mb-6">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <p className="text-xl font-bold text-gray-900">{summaryLoading ? empSkel('w-24') : formatCurrency(summary.revenue, currency)}</p>
              </div>
              <div>
                <p className="text-xs text-gray-500 font-bold uppercase">Jumla Maoni Faida</p>
                <p className="text-xl font-bold text-green-600">{summaryLoading ? empSkel('w-24') : formatCurrency(summary.profit, currency)}</p>
              </div>
              <div>
                <p className="text-xs text-gray-500 font-bold uppercase">Jumla Mauzo</p>
                <p className="text-sm font-bold text-gray-700">{summaryLoading ? empSkel('w-10') : summary.salesCount}</p>
              </div>
              <div>
                <p className="text-xs text-gray-500 font-bold uppercase">Muda Kazini</p>
                <p className="text-sm font-bold text-gray-700">{summaryLoading ? empSkel('w-14') : <>{Math.floor(summary.minutes / 60)}h {summary.minutes % 60}m</>}</p>
              </div>
            </div>
          </div>
                ...
                >
                  ...
                  <div className="grid grid-cols-3 gap-2.5 mb-3">
                    <div className="bg-gray-50 p-3 rounded-2xl border border-gray-100">
                      <div className="flex items-center mb-1">
                        ...
                        <span className="text-[10px] uppercase font-bold text-gray-500">Alianza</span>
                      </div>
                      <p className="font-bold text-gray-900 text-sm">{format(shift.loginTime, 'h:mm a')}</p>
                    </div>
                    <div className="bg-gray-50 p-3 rounded-2xl border border-gray-100">
                      <div className="flex items-center mb-1">
                        <LogOut className="w-3 h-3 text-red-500 mr-1" />
                        <span className="text-[10px] uppercase font-bold text-gray-500">Alimaliza</span>
                      </div>
                      <p className="font-bold text-gray-900 text-sm">
                        {shift.logoutTime ? format(shift.logoutTime, 'h:mm a') : '—'}
                      </p>
                    </div>
                    {/* Sum of presence blocks — absences are NOT counted as worked time. */}
                    <div className="bg-blue-50 p-3 rounded-2xl border border-blue-100">
                      <div className="flex items-center mb-1">
                        <Clock className="w-3 h-3 text-blue-500 mr-1" />
                        <span className="text-[10px] uppercase font-bold text-blue-600">Alikuwepo</span>
                      </div>
                      <p className="font-bold text-blue-900 text-sm">
                        {formatDuration(shift.presence.activeSeconds)}
                      </p>
                    </div>
                  </div>

                  {/* Absences — the hours nobody was minding the shop */}
                  {shift.presence.gaps.length > 0 && (
                    <div className="mb-4 bg-amber-50 border border-amber-100 rounded-2xl p-3">
                      <div className="flex items-center gap-1.5 mb-2">
                        ...
                        <span className="text-[11px] font-black text-amber-900 uppercase tracking-wide">
                          Mapengo ({shift.presence.gaps.length})
                        </span>
                      </div>
                      <ul className="space-y-1">
                        {shift.presence.gaps.slice(0, 4).map((g, gi) => (
                          <li key={gi} className="flex items-center justify-between text-[11.5px] text-amber-900">
                            <span className="font-semibold">
                              {format(g.start, 'h:mm a')} – {format(g.end, 'h:mm a')}
                            </span>
                            <span className="font-black">{formatDuration(g.seconds)}</span>
                          </li>
                        ))}
                      </ul>
                      {shift.presence.blocks.length > 1 && (
                        <p className="text-[10px] text-amber-700 mt-2 leading-relaxed">
                          Alirudi mara {shift.presence.blocks.length} siku hii.
                        </p>
                      )}
                    </div>
                  )}
                  ...
                  <div className="bg-blue-50/50 p-4 rounded-2xl border border-blue-50">
                    ...
                    <div className="flex items-center justify-between text-xs text-blue-700 bg-white/60 p-2 rounded-xl mt-3">
                      ...
                      {shift.presence.activeSeconds > 0 && (
                        <span className="font-semibold flex items-center">
                          <Clock className="w-3 h-3 mr-1"/> {formatDuration(shift.presence.activeSeconds)} kazini
                        </span>
                      )}
                    </div>
                  </div>
  ...
  return (
    <div className="fixed inset-0 bg-gray-50 z-40 flex flex-col overflow-hidden pt-safe pt-safe-standalone">
      {/* Absorbs the iOS ghost click from the tap that opened this screen. */}
      <GhostClickGuard />
      <div className="bg-white px-4 py-4 flex items-center border-b border-gray-100 shadow-sm shrink-0">
        <button 
          onClick={onClose}
