...
import { useScreenTakeover } from '../utils/screenTakeover';
...
export default function EmployeeReports({ onClose }: { onClose: () => void }) {
  // This screen covers the viewport and carries its own top-right control (the
  // permissions gear), so the app's floating notification bell stands down
  // while it is up instead of sitting on top of it.
  useScreenTakeover();
  ...
  return (
    <div className="fixed inset-0 bg-gray-50 z-40 flex flex-col overflow-hidden">
      <div className="bg-white px-4 py-4 flex items-center border-b border-gray-100 shadow-sm shrink-0">
        ...
        {/* The permissions live next to the reports on purpose: this is the
            screen where a boss is already forming an opinion about what their
            staff should and should not be able to do. Boss only — the switches
            are shop-wide and RLS would refuse anyone else's write anyway. */}
        {isBoss() && (
          <button
            onClick={() => setShowPermissions(true)}
            className="p-2.5 rounded-full bg-gray-50 border border-gray-100 text-gray-600 hover:bg-gray-100 hover:text-gray-800 transition-colors shrink-0"
      ...
      {showPermissions && <StaffPermissionsModal onClose={() => setShowPermissions(false)} />}
