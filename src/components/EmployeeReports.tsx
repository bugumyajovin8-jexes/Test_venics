...
export default function EmployeeReports({ onClose }: { onClose: () => void }) {
  ...
  const summary = useMemo(() => {
    // Hours worked come from the (recent-window) shift sessions.
    const minutes = employeeShifts.reduce((acc, shift) => {
      if (shift.logoutTime && shift.logoutTime.getTime() !== shift.loginTime.getTime()) {
        return acc + differenceInMinutes(shift.logoutTime, shift.loginTime);
      }
      return acc;
    }, 0);
