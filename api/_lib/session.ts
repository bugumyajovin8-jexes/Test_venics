...
// installed PWA (macOS/iOS Safari) which localStorage/IndexedDB do not.
...
// Write a JSON response using ONLY raw Node res methods, so it works regardless of
// whether Vercel's res.status()/res.json() helper sugar is present on the response.
export function sendJson(res: any, status: number, body: unknown, setCookie?: string): void {
  try {
    ...
    /* response already sent / unavailable */
  }
}

// Read + JSON-parse a request body from the raw Node stream (Vercel usually
// pre-parses req.body, but we don't depend on that helper being present).
